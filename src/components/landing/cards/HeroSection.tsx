...
export default function HeroSection({ content: initialContent }: HeroSectionProps) {
  ...
  return (
    ...
    >
      ...
      <div className="text-center max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10 py-24 md:py-40">
        <h1 className="text-5xl sm:text-6xl lg:text-8xl font-light mb-16 leading-[1.3]">
          <EditableText
            ...
            className="bg-gradient-to-r from-[#8a44c8] to-[#df0c39] text-transparent bg-clip-text animate-fade-in"
          />
          <EditableText
            content={initialContent?.subtitle || 'Into Finished Songs'}
            onChange={(value) => updateContent('hero.subtitle', value)}
            className="font-bold block mt-6 mb-6 pb-4 bg-gradient-to-r from-[#8a44c8] to-[#df0c39] text-transparent bg-clip-text"
          />
        </h1>
        <EditableText
