...
export default function HeroSection({ content: initialContent }: HeroSectionProps) {
  ...
  return (
    <section 
      className="w-full h-[100vh] h-[100svh] min-h-[600px] flex items-center justify-center relative bg-gradient-to-br from-purple-50 to-white overflow-hidden" 
      aria-label="Hero"
      onMouseEnter={() => setIsHovered(true)}
      onMouseLeave={() => setIsHovered(false)}
    >
      <div className="absolute inset-0 bg-[url('/assets/images/bg/2xl_bg.png')] bg-cover bg-center opacity-5" />
      <PixieDust enabled={!isEditMode && isHovered} particleCount={25} />
